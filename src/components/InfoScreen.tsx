...
export default function InfoScreen({ onInstall, canInstall }: Props) {
  ...
  const prevTeam = () => setTeamIndex(i => (i - 1 + teamMembers.length) % teamMembers.length);
    ...
    <div className="flex flex-col gap-4 px-4 pt-4 pb-6">
      ...
      <div className="rounded-3xl overflow-hidden hero-water">
        <div className="px-5 py-6 flex items-center gap-4">
          {/* Transparent logo — no frames */}
          <img
            src={LOGO}
            alt="La Casa del Pollazo"
            className="w-20 h-20 object-contain flex-shrink-0"
            style={{ filter: 'drop-shadow(0 4px 16px rgba(0,0,0,0.2)) drop-shadow(0 0 8px rgba(255,255,255,0.3))' }}
          />
          <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
        ...
        {[
          ...
          { icon: <MapPin size={16} className="text-red-500" />, label: 'Ubicación', href: MAPS_URL, text: 'El Mirador, Calle Delfín' },
        ].map(item => (
          <a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-3 py-2.5 px-3 bg-gray-50 rounded-xl active:bg-gray-100 transition-colors">
            {item.icon}
            <div>
              <p className="text-[10px] text-gray-400 font-medium">{item.label}</p>
              <p className="text-sm text-gray-800 font-semibold">{item.text}</p>
            </div>
          </a>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-black text-gray-900 text-base">Horario</h3>
        </div>
        {[
          { day: 'Lunes – Viernes', hours: '7:00 AM – 9:00 PM' },
          { day: 'Sábado', hours: '7:00 AM – 10:00 PM' },
      ...
      <div className="bg-gradient-to-r from-orange-50 to-yellow-50 rounded-2xl border border-orange-100 p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-black text-gray-900 text-base">Delivery</h3>
        </div>
        <p className="text-sm text-gray-600 leading-relaxed">
          Entregamos a domicilio en Puerto Ayora. Tiempo estimado: <strong className="text-orange-600">20-40 minutos</strong>.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        ...
        <div className="relative flex items-center justify-center">
          <button onClick={prevTeam} className="absolute left-0 w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center active:scale-90 transition-transform z-10">
            <ChevronLeft size={16} className="text-gray-600" />
          </button>
          <div className="text-center px-10">
            <img
              src={teamMembers[teamIndex].photo}
              alt={teamMembers[teamIndex].name}
              className="w-20 h-20 rounded-full object-cover mx-auto border-4 border-orange-100 shadow-md"
            />
          ...
          <button onClick={nextTeam} className="absolute right-0 w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center active:scale-90 transition-transform z-10">
            <ChevronRight size={16} className="text-gray-600" />
          </button>
        </div>
        <div className="flex justify-center gap-1.5 mt-4">
          {teamMembers.map((_, i) => (
            <button key={i} onClick={() => setTeamIndex(i)} className={`rounded-full transition-all duration-300 ${i === teamIndex ? 'w-5 h-1.5 bg-orange-500' : 'w-1.5 h-1.5 bg-gray-200'}`} />
          ))}
        </div>
      </div>
      ...
      {canInstall && (
        <button
          onClick={onInstall}
          className="w-full flex items-center justify-center gap-2.5 bg-gradient-to-r from-orange-500 to-yellow-400 text-white font-black py-4 rounded-2xl text-sm active:scale-[0.98] transition-transform shadow-lg shadow-orange-200"
        >
          <Download size={18} />
