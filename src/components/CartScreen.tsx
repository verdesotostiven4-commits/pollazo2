...
interface Props { onCheckout: () => void; onNavigate: (s: Screen) => void; }
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-6 gap-4">
        ...
        <button onClick={() => onNavigate('catalog')} className="bg-orange-500 text-white font-bold px-6 py-3 rounded-2xl active:scale-95 transition-transform">
          Ver catálogo
        </button>
      </div>
    );
  ...
  return (
    <div className="flex flex-col pb-4">
        ...
        {items.map(({ product, qty }) => (
          <div key={product.id} className="flex items-center gap-3 bg-white rounded-2xl p-3 border border-gray-100 shadow-sm">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button onClick={() => updateQty(product.id, qty - 1)} className="w-7 h-7 bg-gray-100 rounded-lg flex items-center justify-center active:scale-90 transition-transform">
                <Minus size={12} className="text-gray-600" />
              </button>
              <span className="font-bold text-gray-900 text-sm w-5 text-center">{qty}</span>
              <button onClick={() => updateQty(product.id, qty + 1)} className="w-7 h-7 bg-orange-100 rounded-lg flex items-center justify-center active:scale-90 transition-transform">
                <Plus size={12} className="text-orange-600" />
              </button>
              <button onClick={() => removeItem(product.id)} className="w-7 h-7 bg-red-50 rounded-lg flex items-center justify-center active:scale-90 transition-transform ml-1">
      ...
      <div className="mx-4 mt-3">
        <button onClick={onCheckout} className="w-full bg-gradient-to-r from-orange-500 to-yellow-400 text-white font-black py-4 rounded-2xl text-base active:scale-[0.98] transition-transform shadow-lg shadow-orange-200">
