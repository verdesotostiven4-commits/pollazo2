...
export default function LiveMetrics() {
  const [metrics, setMetrics] = useState<{ total_orders: number; happy_customers: number }>({ total_orders: 847, happy_customers: 612 });

  useEffect(() => {
    supabase.from('metrics').select('*').then(({ data }) => {
      if (data) {
        const map: Record<string, number> = {};
        (data as Metric[]).forEach(m => { map[m.id] = m.value; });
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {[
        { label: 'Pedidos realizados', value: metrics.total_orders, icon: '📦' },
        { label: 'Clientes felices', value: metrics.happy_customers, icon: '😊' },
      ].map(m => (
